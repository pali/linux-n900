...
/*
 * soc level interface for requesting runtime DAI power level change
 */
int snd_soc_request_power_level(struct snd_pcm_substream *substream,
				enum snd_soc_power_level level)
{
	...
	if (cpu_dai->ops->request_power_level)
		ret = cpu_dai->ops->request_power_level(substream, level);

	return ret;
}
EXPORT_SYMBOL_GPL(snd_soc_request_power_level);
