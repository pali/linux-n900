...
#define BURST_SIZE_FROM_UTHR(uthr, playrate, burstrate) \
	(((uthr) * ((burstrate * 5000) / (burstrate - playrate))) / 5000)
...
struct tlv320dac33_priv {
	...
	enum dac33_fifo_modes user_fifo_mode; /* FIFO mode selection */
	enum dac33_fifo_modes fifo_mode; /* Active FIFO mode */
	unsigned int fallback_to_bypass_time;	/* Fallback threshold to FIFO
						 * bypass mode */
	unsigned int nsample;		/* burst read amount from host */
	int mode1_latency;		/* latency caused by the i2c writes in
					 * us */
	int mode7lp_latency;		/* latency caused by the system wakeup
					 * from lower power state in us */
	int mode7lp_timer_us;		/* us to schedule the timer */
	unsigned char mode7lp_qos_placed:1;
	...
	unsigned int burst_time_us;	/* Time to burst read n number of
					 * samples */
	...
	unsigned int athr_time_ms;	/* ATHR time */
...
static int dac33_hard_power(struct snd_soc_codec *codec, int power)
{
	...
	if (power) {
		...
		if (dac33->mode7lp_qos_placed) {
			pm_qos_remove_requirement(PM_QOS_CPU_DMA_LATENCY,
						  "dac33");
			dac33->mode7lp_qos_placed = 0;
		}
...
static int dac33_get_athr_time(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct tlv320dac33_priv *dac33 = snd_soc_codec_get_drvdata(codec);

	ucontrol->value.integer.value[0] = dac33->athr_time_ms;

	return 0;
}

static int dac33_set_athr_time(struct snd_kcontrol *kcontrol,
			struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct tlv320dac33_priv *dac33 = snd_soc_codec_get_drvdata(codec);
	...
	if (dac33->athr_time_ms == ucontrol->value.integer.value[0])
		return 0;

	if (ucontrol->value.integer.value[0] < 0 ||
		ucontrol->value.integer.value[0] > 120)
		ret = -EINVAL;
	else
		dac33->athr_time_ms = ucontrol->value.integer.value[0];
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = dac33->user_fifo_mode;
			 ...
			 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (dac33->user_fifo_mode == ucontrol->value.integer.value[0])
		return 0;
	...
	if (ucontrol->value.integer.value[0] < 0 ||
		...
		dac33->user_fifo_mode = ucontrol->value.integer.value[0];
...
static const char *dac33_fifo_mode_texts[] = {
	"Bypass", "Mode 1", "Mode 7", "Mode 7 Low Power"
};
...
static const struct snd_kcontrol_new dac33_snd_controls[] = {
	...
	/* HACK: ATHR control */
	SOC_SINGLE_EXT("ATHR time", 0, 0, 120, 0,
		dac33_get_athr_time, dac33_set_athr_time),
};
...
static inline void dac33_prefill_handler(struct tlv320dac33_priv *dac33)
{
	...
	case DAC33_FIFO_MODE7LP:
		...
		dac33->t_stamp1 = ktime_to_us(ktime_get());
		/* Move back the timestamp */
		dac33->t_stamp1 -= dac33->mode7lp_latency;
		dac33->t_stamp1 += SAMPLES_TO_US(dac33->burst_rate,
						DAC33_MODE7_MARGIN);
		...
		dac33_write16(codec, DAC33_PREFILL_MSB,
				DAC33_THRREG(DAC33_MODE7_MARGIN));
		/* Enable Alarm Threshold IRQ with a delay */
		delay = dac33->burst_time_us + 1000;
		usleep_range(delay, delay + 500);
		dac33_write(codec, DAC33_FIFO_IRQ_MASK, DAC33_MAT);

		/*
		 * place qos, so we should be guarantied that we are
		 * not going to miss the DAC33 streaming start.
		 */
		if (!dac33->mode7lp_qos_placed) {
			pm_qos_add_requirement(PM_QOS_CPU_DMA_LATENCY, "dac33",
						dac33->mode7lp_latency);
			dac33->mode7lp_qos_placed = 1;
		}
		break;
	default:
...
static inline void dac33_playback_handler(struct tlv320dac33_priv *dac33)
{
	...
	case DAC33_FIFO_MODE7:
		...
		break;
	case DAC33_FIFO_MODE7LP:
		time_delta = ktime_to_us(ktime_get()) - dac33->t_stamp1;
		if (likely(time_delta >= dac33->mode7lp_timer_us)) {
			/* Set the CPU side to inactive */
			snd_soc_request_power_level(dac33->substream,
						    SND_SOC_PM_INACTIVE);
		}
		break;
	default:
...
static void dac33_work(struct work_struct *work)
{
	...
	case DAC33_FLUSH:
		...
		/* Remove the qos */
		if (dac33->fifo_mode == DAC33_FIFO_MODE7LP &&
		    dac33->mode7lp_qos_placed) {
			pm_qos_remove_requirement(PM_QOS_CPU_DMA_LATENCY,
						  "dac33");
			dac33->mode7lp_qos_placed = 0;
		}
...
static enum hrtimer_restart dac33_hrtimer_handler(struct hrtimer *handle)
{
	struct tlv320dac33_priv *dac33 = container_of(handle,
			struct tlv320dac33_priv, hrtimer);
...
static irqreturn_t dac33_interrupt_handler(int irq, void *dev)
{
	...
	case DAC33_FIFO_MODE7:
		spin_lock_irqsave(&dac33->lock, flags);
		dac33->t_stamp1 = time_now;
		spin_unlock_irqrestore(&dac33->lock, flags);
		break;
	case DAC33_FIFO_MODE7LP:
		/* Filter out the ghost ATHM interrupts based on timestamps,
		 * and non playback time interrupts */
		if (dac33->state == DAC33_PLAYBACK &&
		    (time_now - dac33->t_stamp1) >
		     dac33->mode7lp_timer_us) {
			/* Only do this if it makes sense */
			if (dac33->mode7lp_timer_us) {
				snd_soc_request_power_level(dac33->substream,
							    SND_SOC_PM_ACTIVE);
				hrtimer_start(&dac33->hrtimer, ktime_set(0,
						dac33->mode7lp_timer_us * 1000),
						HRTIMER_MODE_REL);
			}
			spin_lock_irqsave(&dac33->lock, flags);
			dac33->t_stamp1 = time_now;
			   ...
			   struct snd_soc_dai *dai)
{
	...
	dac33->fifo_mode = dac33->user_fifo_mode;
	if (dac33->fifo_mode) {
		unsigned int period_time = SAMPLES_TO_US(params_rate(params),
					    params_period_size(params));

		if (period_time <= dac33->fallback_to_bypass_time)
...
static int dac33_prepare_chip(struct snd_pcm_substream *substream)
{
	...
	case DAC33_FIFO_MODE1:
		...
		break;
	case DAC33_FIFO_MODE7LP:
		/* Configure the alarm threshold level */
		dac33_write16(codec, DAC33_ATHR_MSB,
			      DAC33_THRREG(dac33->alarm_threshold));
	case DAC33_FIFO_MODE7:
		/*
...
static void dac33_calculate_times(struct snd_pcm_substream *substream)
{
	...
	unsigned int nsample_limit, cycle_samples;
	...
	case DAC33_FIFO_MODE1:
		...
		dac33->burst_time_us = SAMPLES_TO_US(dac33->burst_rate,
						      dac33->nsample);
		dac33->t_stamp1 = 0;
		dac33->t_stamp2 = 0;
		break;
	case DAC33_FIFO_MODE7LP:
		...
		dac33->mode7_us_to_lthr = SAMPLES_TO_US(rate,
					dac33->uthr - DAC33_MODE7_MARGIN);

		cycle_samples = BURST_SIZE_FROM_UTHR(dac33->uthr, rate,
						     dac33->burst_rate) - 9;

		/* 10ms on top of alarm lower threshold */
		dac33->mode7lp_latency = dac33->athr_time_ms * 1000;
		...
		/*
		 * Calculate the time for the timer to set the host to
		 * inactive state (after the burst finished)
		 * Use this time to filter out the ghost interrupts
		 */
		dac33->mode7lp_timer_us = dac33->mode7lp_latency +
					 dac33->burst_time_us + 1000;

		/* Disable the PM in case of small period size */
		if (dac33->mode7lp_timer_us >
		    (SAMPLES_TO_US(rate, cycle_samples) - dac33->mode7lp_latency))
			dac33->mode7lp_timer_us = 0;
			...
			struct snd_soc_dai *dai)
{
	...
	case DAC33_FIFO_MODE1:
		...
		if (t0 > t1) {
			...
			if (likely(dac33->alarm_threshold > samples_out))
				...
				delay = 0;
		} else if ((t_now - t1) <= dac33->burst_time_us) {
			/*
			 * Phase 2:
	...
	case DAC33_FIFO_MODE7LP:
		...
		time_delta = t_now - t0;
		if (time_delta <= dac33->mode7lp_latency) {
			/*
			 * Phase 1:
			 * Darining, after ATHR level has been reached
			 */
			delay = dac33->alarm_threshold;
			samples_out = US_TO_SAMPLES(
					substream->runtime->rate,
					time_delta);
			if (unlikely(samples_out > delay))
				delay = DAC33_MODE7_MARGIN;
			else
				delay -= samples_out;
		} else if (time_delta <= (dac33->mode7lp_latency +
					  dac33->burst_time_us)) {
			/*
			 * Phase 2:
			 * During burst operation
			 */
			time_delta -= dac33->mode7lp_latency;
			samples_out = US_TO_SAMPLES(
					substream->runtime->rate,
					time_delta);
			samples_in = US_TO_SAMPLES(
				     ...
				     const struct i2c_device_id *id)
{
	...
	dac33->mode7lp_latency = pdata->mode7lp_latency;
	if (!dac33->mode7lp_latency)
		dac33->mode7lp_latency = 10000; /* 10ms */
	dac33->athr_time_ms = dac33->mode7lp_latency / 1000;

	dac33->fallback_to_bypass_time = pdata->fallback_to_bypass_time;
	if (!dac33->fallback_to_bypass_time)
		dac33->fallback_to_bypass_time = 40000; /* 40ms */

	dac33->irq = client->irq;
	/* Disable FIFO use by default */
	dac33->user_fifo_mode = DAC33_FIFO_BYPASS;
	...
	if (dac33->irq >= 0) {
		...
		if (dac33->irq != -1) {
			...
			hrtimer_init(&dac33->hrtimer, CLOCK_MONOTONIC,
				     HRTIMER_MODE_REL);
			dac33->hrtimer.function = dac33_hrtimer_handler;

			/* Enable wake-up for the IRQ */
			enable_irq_wake(dac33->irq);
		}
	}
