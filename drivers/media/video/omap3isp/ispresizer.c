...
/* Memory alignment. Both input and output addresses and line sizes must be
 * aligned on a 32-byte boundary.
 */
#define RSZ_IN_ALIGNMENT		32
#define RSZ_OUT_ALIGNMENT		32
				    ...
				    const struct v4l2_mbus_framefmt *output)
{
	...
	if (res->input == RESIZER_INPUT_MEMORY) {
		unsigned int bpl = ALIGN(input->width * 2, RSZ_IN_ALIGNMENT);

		/* Calculate additional offset for crop */
		res->crop_offset = res->crop.active.top * bpl
...
static void resizer_configure(struct isp_res_device *res)
{
	...
	if (res->input == RESIZER_INPUT_VP)
		...
		resizer_set_input_offset(res,
			ALIGN(informat->width * 2, RSZ_IN_ALIGNMENT));
	...
	/* RESZ_PAD_SOURCE */
	resizer_set_output_offset(res,
		ALIGN(outformat->width * 2, RSZ_OUT_ALIGNMENT));
...
static int resizer_init_entities(struct isp_res_device *res)
{
	...
	res->video_in.bpl_alignment = RSZ_IN_ALIGNMENT;
	...
	res->video_out.bpl_alignment = RSZ_OUT_ALIGNMENT;
