...
#define NCM_MTU			8192
#define NTB_DEFAULT_IN_SIZE	(NCM_MTU + 512)
#define NTB_OUT_SIZE		(NCM_MTU + 512)
...
static struct usb_cdc_ether_desc ecm_desc __initdata = {
	...
	.wMaxSegmentSize =	cpu_to_le16(NCM_MTU),
...
static void ncm_ep0out_size_complete(struct usb_ep *ep, struct usb_request *req)
{
...
static void ncm_ep0out_mtu_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	mtu = get_unaligned_le16(req->buf);
	if (mtu <= ETH_HLEN || mtu > ncm->mss) {
		DBG(cdev, "Got wrong MTU (%d) from host\n", mtu);
		goto invalid;
	}

	ncm->port.mtu = mtu;
	gether_update_mtu(&ncm->port);
	VDBG(cdev, "Set NTB MTU %d\n", mtu);
	return;
...
static int ncm_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
		...
		| USB_CDC_SET_NTB_INPUT_SIZE:
	{
		if (w_length != 4 || w_value != 0 || w_index != ncm->ctrl_id)
			goto invalid;
		req->complete = ncm_ep0out_size_complete;
		req->length = w_length;
		req->context = f;
	...
	case ((USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		put_unaligned_le16(ncm->port.mtu, req->buf);
		value = 2;
		break;
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		if (gether_is_connected(&ncm->port))
			goto invalid;

		req->complete = ncm_ep0out_mtu_complete;
		req->length = w_length;
		req->context = f;
...
int __init ncm_bind_config(struct usb_configuration *c, u8 ethaddr[ETH_ALEN])
{
	...
	ncm_reset_values(ncm);
	ncm->mss = le32_to_cpu(ecm_desc.wMaxSegmentSize);
	ncm->port.is_fixed = true;
	ncm->port.mtu = NCM_MTU;
