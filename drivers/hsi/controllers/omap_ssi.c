 ...
 * @ready_rx_pad: PAD reg offset for ready rx line. DVFS workaround
 * @ready_tx_pad: PAD reg offset for ready rx line. DVFS workaround
...
static int ssi_update_divisor(struct omap_ssi_port *omap_port, void *data)
{
	u32 *div = data;
...
static int ssi_rdytx_pad(struct omap_ssi_port *omap_port, void *data)
{
	u16 *muxpull = data;
	u16 val;

	val = omap_ctrl_readw(omap_port->ready_tx_pad);
	/* Update the mux and the pull */
	val = (val & ~0x1f) | *muxpull;
	omap_ctrl_writew(val, omap_port->ready_tx_pad);
	/* OCP barrier */
	val = omap_ctrl_readw(omap_port->ready_tx_pad);
...
static int ssi_rdyrx_pad(struct omap_ssi_port *omap_port, void *data)
{
	u16 *mux = data;
	u16 val;

	val = omap_ctrl_readw(omap_port->ready_rx_pad);
	/* Update only the mux value */
	val = (val & ~OMAP34XX_MUX_MODE7) | *mux;
	omap_ctrl_writew(val, omap_port->ready_rx_pad);
	/* OCP barrier */
	val = omap_ctrl_readw(omap_port->ready_rx_pad);
...
static int ssi_disable_waketasklet(struct omap_ssi_port *omap_port, void *data)
{
...
static int ssi_enable_waketasklet(struct omap_ssi_port *omap_port, void *data)
{
								...
								void *data)
{
	...
	case CLK_PRE_RATE_CHANGE:
		...
		ssi_for_each_port(ssi, NULL, ssi_disable_waketasklet);
		dev_dbg(&ssi->device, "pre rate change\n");
		/* Safe mode + enable pull down -> ready_tx low */
		val = OMAP34XX_MUX_MODE7 | OMAP2_PULL_ENA;
		ssi_for_each_port(ssi, &val, ssi_rdytx_pad);
		/* Reroute to gpio with output low -> ready_rx low */
		val = OMAP34XX_MUX_MODE4;
		ssi_for_each_port(ssi, &val, ssi_rdyrx_pad);
	...
	case CLK_POST_RATE_CHANGE:
		dev_dbg(&ssi->device, "post rate change\n");
		omap_ssi->fck_rate = clk_data->rate / 1000; /* KHz */
		div = ssi_calculate_div(ssi);
		ssi_for_each_port(ssi, &div, ssi_update_divisor);
		ssi_for_each_port(ssi, NULL, ssi_restore_divisor);
		val = OMAP34XX_MUX_MODE1;
		ssi_for_each_port(ssi, &val, ssi_rdyrx_pad);
		ssi_for_each_port(ssi, &val, ssi_rdytx_pad);
		ssi_for_each_port(ssi, NULL, ssi_enable_waketasklet);
		break;
	default:
		break;
	}
...
static int ssi_flush(struct hsi_client *cl)
{
	...
	/* Safe mode + enable pull down -> ready_tx low */
	val = OMAP34XX_MUX_MODE7 | OMAP2_PULL_ENA;
	ssi_rdytx_pad(omap_port, &val);
	/* Reroute to gpio with output low -> ready_rx low */
	val = OMAP34XX_MUX_MODE4;
	ssi_rdyrx_pad(omap_port, &val);
	spin_unlock_bh(&omap_port->lock);
	/* Wait for racing frames */
	udelay(1);
	/* At this point all SSI communication is stopped */
	spin_lock_bh(&omap_port->lock);
	...
	/* Flush all errors */
	val = __raw_readl(ssr + SSI_SSR_ERROR_REG);
	__raw_writel(val, ssr + SSI_SSR_ERRORACK_REG);
	/* Flush break */
...
static void ssi_wake_tasklet(unsigned long ssi_port)
{
	...
	if (ssi_wakein(port)) {
		 ...
		 *
		 * A second workaround (port->claimed check) is used to ignore
		 * the wakelines when no clients are using the omap_ssi
		 * Otherwise, CORE power domain will not go into retention.
		 */
		spin_lock(&omap_port->lock);
		if (!omap_port->wkin_cken && port->claimed) {
					...
					struct platform_device *pd)
{
	...
	for (i = 0; i < ssi->num_ports; i++) {
		...
		omap_port->ready_rx_pad =
				omap_ssi_pdata->port_data[i].ready_rx_pad;
		omap_port->ready_tx_pad =
				omap_ssi_pdata->port_data[i].ready_tx_pad;
						...
						struct platform_device *pd)
{
	...
	omap_ssi->ick = ssi_devm_clk_get(&pd->dev, "ssi_ick", NULL);
