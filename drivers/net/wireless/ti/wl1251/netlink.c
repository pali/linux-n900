...
static int wl1251_nl_test_cmd(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (answer) {
		...
		if (nla_put_string(msg, WL1251_NL_ATTR_IFNAME,
			       nla_data(info->attrs[WL1251_NL_ATTR_IFNAME])))
			goto nla_put_failure;
		if (nla_put(msg, WL1251_NL_ATTR_CMD_TEST_ANSWER,
			sizeof(*cmd), cmd))
			goto nla_put_failure;
...
static int wl1251_nl_nvs_push(struct sk_buff *skb, struct genl_info *info)
{
	...
	wl->nvs = kmemdup(nla_data(info->attrs[WL1251_NL_ATTR_NVS_BUFFER]),
			  wl->nvs_len, GFP_KERNEL);
	if (!wl->nvs) {
...
{
	return genl_register_family_with_ops(&wl1251_nl_family, wl1251_nl_ops,
					     ARRAY_SIZE(wl1251_nl_ops));
}
