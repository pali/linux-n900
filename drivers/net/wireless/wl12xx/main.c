...
static struct conf_drv_settings default_conf = {
	...
	.scan = {
		...
		.num_probe_reqs               = 2,
		.max_channels                 = WL1271_SCAN_MAX_CHANNELS,
		.inter_delay                  = 0,
	},
	.rf = {
...
power_off:
	...
	/*
	 * In hw recovery cases packets may build up in the TX queue and
	 * the queues may be stopped. Recover from that.
	 */
	if (wl->tx_queue_count >= WL1271_TX_QUEUE_HIGH_WATERMARK)
		set_bit(WL1271_FLAG_TX_QUEUE_STOPPED, &wl->flags);
	wl1271_tx_reset(wl);
...
static void __wl1271_op_remove_interface(struct wl1271 *wl)
{
	...
	cancel_delayed_work_sync(&wl->scan_work);
	cancel_work_sync(&wl->irq_work);
	cancel_work_sync(&wl->tx_work);
	cancel_delayed_work_sync(&wl->pspoll_work);
	cancel_delayed_work_sync(&wl->elp_work);
	cancel_work_sync(&wl->ampdu_action_work);
				       ...
				       u32 changed)
{
	...
	if (changed & BSS_CHANGED_ASSOC) {
		if (bss_conf->assoc) {
			...
			if (wl->pdata && wl->pdata->request_perf)
				wl->pdata->request_perf(&wl->plat_dev->dev,
			...
			ret = wl1271_ssid_set(wl, wl->probereq, ieoffset);
			if (ret < 0)
				goto out_sleep;
			...
			/* relieve performance requirements */
			if (wl->pdata && wl->pdata->request_perf)
				wl->pdata->request_perf(&wl->plat_dev->dev,
							WL12XX_PERF_DONT_CARE);
...
static void wl1271_ampdu_action_work(struct work_struct *work)
{
	struct wl1271 *wl =
		container_of(work, struct wl1271, ampdu_action_work);
	int ret;
	...
	case IEEE80211_AMPDU_RX_START:
		if (wl->ba_support) {
			ret = wl1271_acx_set_ba_receiver_session(wl, wl->ba.tid,
								 *wl->ba.ssn,
								 true);
			if (!ret)
				wl->ba_rx_bitmap |= BIT(wl->ba.tid);
		} else {
			ret = -ENOTSUPP;
		}

		/* in the meanwhile, if BA were denied, stop BA */
		if (test_bit(WL1271_FLAG_NO_HT_BA, &wl->flags))
			wl1271_event_ba(wl);
			   ...
			   struct ieee80211_sta *sta, u16 tid, u16 *ssn)
{
	...
	switch (action) {
	case IEEE80211_AMPDU_RX_START:
		/* FIXME: these are racy, but let it be for now */
		wl->ba.action = action;
		wl->ba.tid = tid;
		wl->ba.ssn = ssn;
		ieee80211_queue_work(wl->hw, &wl->ampdu_action_work);
		break;
	case IEEE80211_AMPDU_RX_STOP:
		/* FIXME: these are racy, but let it be for now */
		wl->ba.action = action;
...
static ssize_t wl1271_sysfs_show_scan_delay(struct device *dev,
					    struct device_attribute *attr,
					    char *buf)
{
	struct wl1271 *wl = dev_get_drvdata(dev);
	...
	mutex_lock(&wl->mutex);
	len = snprintf(buf, len, "%d ms\n", wl->conf.scan.inter_delay);
	mutex_unlock(&wl->mutex);
...
static ssize_t wl1271_sysfs_store_scan_delay(struct device *dev,
					     struct device_attribute *attr,
					     const char *buf, size_t count)
{
	struct wl1271 *wl = dev_get_drvdata(dev);
	...
	if (ret < 0 || res > USHORT_MAX) {
		wl1271_warning("incorrect value written to scan_delay");
		return count;
	}

	mutex_lock(&wl->mutex);

	wl->conf.scan.inter_delay = res;

	mutex_unlock(&wl->mutex);
	return count;
}

static DEVICE_ATTR(scan_delay, S_IRUGO | S_IWUSR,
		   wl1271_sysfs_show_scan_delay,
		   wl1271_sysfs_store_scan_delay);

static ssize_t wl1271_sysfs_show_scan_size(struct device *dev,
					   struct device_attribute *attr,
					   char *buf)
{
	struct wl1271 *wl = dev_get_drvdata(dev);
...
static ssize_t wl1271_sysfs_store_scan_size(struct device *dev,
					    struct device_attribute *attr,
					    const char *buf, size_t count)
{
	struct wl1271 *wl = dev_get_drvdata(dev);
	...
	if (ret < 0 || res > WL1271_SCAN_MAX_CHANNELS) {
		wl1271_warning("incorrect value written to scan_size");
		return count;
	}

	mutex_lock(&wl->mutex);

	wl->conf.scan.max_channels = res;

	mutex_unlock(&wl->mutex);
	return count;
}

static DEVICE_ATTR(scan_size, S_IRUGO | S_IWUSR,
		   wl1271_sysfs_show_scan_size,
		   wl1271_sysfs_store_scan_size);
...
static ssize_t wl1271_sysfs_show_allow_adhoc(struct device *dev,
					     struct device_attribute *attr,
					     char *buf)
{
	struct wl1271 *wl = dev_get_drvdata(dev);
...
static ssize_t wl1271_sysfs_store_allow_adhoc(struct device *dev,
					      struct device_attribute *attr,
					      const char *buf, size_t count)
{
	struct wl1271 *wl = dev_get_drvdata(dev);
...
static DEVICE_ATTR(allow_adhoc, S_IRUGO | S_IWUSR,
		   wl1271_sysfs_show_allow_adhoc,
		   wl1271_sysfs_store_allow_adhoc);
...
struct ieee80211_hw *wl1271_alloc_hw(void)
{
	...
	INIT_WORK(&wl->ampdu_action_work, wl1271_ampdu_action_work);
	INIT_DELAYED_WORK(&wl->scan_complete_work, wl1271_scan_complete_work);
	INIT_DELAYED_WORK(&wl->scan_work, wl1271_scan_work);
	...
	/* Create sysfs file to get inter scan delay */
	ret = device_create_file(&wl->plat_dev->dev, &dev_attr_scan_delay);
	if (ret < 0) {
		wl1271_error("failed to create sysfs file scan_delay");
		goto err_hw_pg_ver;
	}

	/* Create sysfs file to get max scan channels */
	ret = device_create_file(&wl->plat_dev->dev, &dev_attr_scan_size);
	if (ret < 0) {
		wl1271_error("failed to create sysfs file scan_size");
		goto err_scan_delay;
	}

	/* Create sysfs file to allow/deny adhoc */
	ret = device_create_file(&wl->plat_dev->dev, &dev_attr_allow_adhoc);
	if (ret < 0) {
		wl1271_error("failed to create sysfs file allow_adhoc");
		goto err_scan_size;
	}
...
err_scan_size:
	device_remove_file(&wl->plat_dev->dev, &dev_attr_scan_size);

err_scan_delay:
	device_remove_file(&wl->plat_dev->dev, &dev_attr_scan_delay);
