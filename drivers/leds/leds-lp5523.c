...
/* Following are microseconds */
#define LP5523_STARTUP_TIME	500
#define LP5523_PROGRAM_STARTUP	3000
#define LP5523_CH_SETTLING_MIN	2000
#define LP5523_CH_SETTLING_MAX	3000
#define LP5523_ADC_TIME_MIN	3000 /* ADC conversion time */
#define LP5523_ADC_TIME_MAX	4000
#define LP5523_EN_DOWN_TIME	1000 /* Time how long enable is down at probe */
...
static int lp5523_detect(struct i2c_client *client)
{
	...
	if (ret)
		return ret;
	if (buf == LP5523_ENABLE)
		return 0;
	else
		return -ENODEV;
}

static void lp5523_reset(struct i2c_client *client)
{
	s32 ret;
	union i2c_smbus_data data;
	data.byte = 0xff;
	/* Known to cause timeout */
	ret = i2c_smbus_xfer(client->adapter, client->addr,
		client->flags | I2C_M_IGNORE_TIMEOUT,
		I2C_SMBUS_WRITE, LP5523_REG_RESET,
		I2C_SMBUS_BYTE_DATA, &data);
	/*
	 * Chip causes normally I2C timeout with sw reset command.
	 * Short delay after the transfer in case of NO timeout
	 */
	if (ret != -ETIMEDOUT)
		msleep(10);
}
...
static int lp5523_configure(struct i2c_client *client)
{
	...
	usleep_range(LP5523_STARTUP_TIME, LP5523_STARTUP_TIME * 2);
	...
	usleep_range(LP5523_PROGRAM_STARTUP, LP5523_PROGRAM_STARTUP * 2);
			       ...
			       char *buf)
{
	...
	usleep_range(LP5523_ADC_TIME_MIN, LP5523_ADC_TIME_MAX);
	ret = lp5523_read(chip->client, LP5523_REG_STATUS, &status);
	...
	for (i = 0; i < LP5523_LEDS; i++) {
		...
		usleep_range(LP5523_CH_SETTLING_MIN, LP5523_CH_SETTLING_MAX);
		lp5523_write(chip->client,
			...
			const struct i2c_device_id *id)
{
	...
	if (pdata->enable) {
		pdata->enable(0);
		usleep_range(LP5523_EN_DOWN_TIME, LP5523_EN_DOWN_TIME * 2);
		pdata->enable(1);
		usleep_range(LP5523_STARTUP_TIME, LP5523_STARTUP_TIME * 2);
	}

	/* Detection may fail if the chip is not reseted */
	lp5523_reset(client);
