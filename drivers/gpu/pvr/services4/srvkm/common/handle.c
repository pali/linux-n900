...
static PVRSRV_ERROR IncreaseHandleArraySize(PVRSRV_HANDLE_BASE *psBase, IMG_UINT32 ui32Delta)
{
	...
	PVRSRV_ERROR eError;
	struct sHandle *psHandle;
	IMG_UINT32 ui32DeltaRounded = ROUND_UP_TO_MULTIPLE(ui32Delta, HANDLE_BLOCK_SIZE);
	IMG_UINT32 ui32NewTotalHandCount = psBase->ui32TotalHandCount + ui32DeltaRounded;
;
	...
	for(psHandle = psNewHandleArray + psBase->ui32TotalHandCount;
		psHandle < psNewHandleArray + ui32NewTotalHandCount;
		psHandle++)
	{
		psHandle->eType = PVRSRV_HANDLE_TYPE_NONE;
