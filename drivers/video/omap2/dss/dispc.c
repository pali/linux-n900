...
static int color_mode_to_ps(enum omap_color_mode color_mode)
{
	switch (color_mode) {
	case OMAP_DSS_COLOR_YUV2:
	case OMAP_DSS_COLOR_UYVY:
		return 4;
	default:
		return DIV_ROUND_UP(color_mode_to_bpp(color_mode), 8);
	}
}
...
{
	u8 ps = color_mode_to_ps(color_mode);
	u16 row_slop;
	...
	row_slop = xinc - (roundup(width, xinc) - width);
	 ...
	 */
	if (mirror) {
		*offset1 = screen_width * (height - 1) * ps;;
		*offset0 = *offset1 - field_offset * screen_width * ps;
		*row_inc = pixinc(-yinc * screen_width
				  -(width - row_slop), ps);
		*pix_inc = pixinc(xinc, ps);
	} else {
		*offset1 = 0;
		*offset0 = *offset1 + field_offset * screen_width * ps;
		*row_inc = pixinc(yinc * screen_width
		...
		s32 *row_inc, s32 *pix_inc)
{
	...
	case OMAP_DSS_ROT_90:
		...
		*row_inc = pixinc(yinc
				  +(fbh - row_slop) * screen_width, ps);
		*pix_inc = pixinc(-xinc * screen_width, ps);
		break;
	case OMAP_DSS_ROT_180:
...
static int check_horiz_blanking(u16 width, u16 height,
		u16 out_width, u16 out_height,
		enum omap_channel channel_out)
{
	static const u8 limits[3] = { 8, 10, 20 };
	unsigned int lclk, pclk;
	struct omap_overlay_manager *mgr =
		omap_dss_get_overlay_manager(channel_out);
	...
	dssdev = mgr->device;
	if (!dssdev || !dssdev->driver->get_timings)
		return -ENODEV;

	/* Assume FIFO stall/handcheck saves the day. */
	if (dssdev->caps & OMAP_DSS_DISPLAY_CAP_MANUAL_UPDATE)
		return 0;

	dssdev->driver->get_timings(mgr->device, &t);
	if (!t.pixel_clock)
		return -EINVAL;

	pclk = t.pixel_clock * 1000;
	lclk = dispc_lclk_rate();
	...
	val = div_u64((u64)(t.hbp + t.hsw + t.hfp) * lclk, pclk);

	DSSDBG("(hbp + hsw + hfp) * lclk / pclk = %llu (limit = %u)\n",
	       val, limits[i]);
		...
		enum omap_color_mode color_mode, bool five_taps)
{
	...
	DSSDBG("(nonactive - pos_x) * lclk / pclk = %llu,"
	       " max(0, ds - 2) * width = %d\n",
	...
	DSSDBG("nonactive * lclk / pclk = %llu, max(0, ds - 1) * width = %d\n",
...
static int check_source_width(u16 width,
		enum omap_color_mode color_mode,
		bool five_taps)
{
...
static unsigned long calc_lclk_five_taps(u16 width, u16 height,
		...
		enum omap_channel channel_out)
{
	...
	if (height > out_height) {
		unsigned int ppl = t.x_res;
		if (!ppl)
			return 0;

		lclk = div_u64(pclk * height * out_width,
			       2 * out_height * ppl);
		...
		if (height > 2 * out_height) {
			...
			tmp = div_u64(pclk * (height - 2 * out_height) * out_width,
				      2 * out_height * (ppl - out_width));
			lclk = max(lclk, tmp);
		}
	}

	if (width > out_width) {
		tmp = div_u64(pclk * width, out_width);
		lclk = max(lclk, tmp);
...
static unsigned long calc_lclk(u16 width, u16 height,
		u16 out_width, u16 out_height,
		enum omap_channel channel_out)
{
	unsigned int hf, vf;
	...
	return t.pixel_clock * 1000 * vf * hf;
}

static int check_color_mode(enum omap_plane plane,
		enum omap_color_mode color_mode)
{
	if (plane == OMAP_DSS_GFX) {
		switch (color_mode) {
...
static u16 align_down(u16 x, enum omap_color_mode color_mode)
{
	switch (color_mode) {
	case OMAP_DSS_COLOR_CLUT1:
		return x & ~7;
...
static u16 align_up(u16 x, enum omap_color_mode color_mode)
{
	switch (color_mode) {
	case OMAP_DSS_COLOR_CLUT1:
		return ALIGN(x, 8);
...
static void align_size(u16 *width, u16 *height,
		enum omap_color_mode color_mode,
		...
		u8 rotation)
{
	...
	if (rotation_type == OMAP_DSS_ROT_VRFB) {
		*width = align_up(*width, color_mode);
		return;
	} else {
		switch (rotation) {
		case OMAP_DSS_ROT_0:
		...
		case OMAP_DSS_ROT_90:
		case OMAP_DSS_ROT_270:
			*height = align_up(*height, color_mode);
			break;
		}
	}
}

static u32 calc_fb_offset(u8 rotation,
		u16 screen_width,
		enum omap_dss_rotation_type rotation_type,
		enum omap_color_mode color_mode,
		u16 offset_x, u16 offset_y)
{
	u8 ps = color_mode_to_ps(color_mode);
	...
	return (offset_y * screen_width + offset_x) * ps;
}
		...
		u16 xinc, u16 yinc)
{
	...
	offset_x = align_down(offset_x, color_mode);

	paddr += calc_fb_offset(rotation, screen_width, rotation_type,
				color_mode, offset_x, offset_y);

	align_size(&width, &height,
		   color_mode, rotation_type, rotation);

	frame_width = width;
	frame_height = height;

	width = DIV_ROUND_UP(width, xinc);
	height = DIV_ROUND_UP(height, yinc);

	align_size(&width, &height,
		   color_mode, rotation_type, rotation);

	if (ilace && height == out_height &&
	    rotation == OMAP_DSS_ROT_0 && yinc == 1)
		fieldmode = 1;
	...
	if (check_color_mode(plane, color_mode))
		return -EINVAL;
	...
	if (plane == OMAP_DSS_GFX) {
		...
		unsigned long lclk = 0;

		if (out_width < width / maxdownscale ||
		    out_height < height / maxdownscale)
			return -EAGAIN;
		...
		if (check_horiz_blanking(width, height, out_width, out_height,
				channel_out)) {
			DSSDBG("horizontal blanking too tight\n");
			return -EAGAIN;
		}

		/* Prefer to use 5-tap filter */
		five_taps = cpu_is_omap34xx() && height > out_height;

again:
		if (check_horiz_timing(channel_out, pos_x, width, height,
				out_width, out_height, color_mode, five_taps)) {
			/* Try again with 3-tap if possible */
			if (five_taps && height <= out_height * 2) {
				five_taps = false;
				goto again;
			} else {
		...
		if (check_source_width(width, color_mode, five_taps)) {
			/* Try again with 3-tap if possible */
			if (five_taps && height <= out_height * 2) {
				five_taps = false;
				goto again;
			} else {
				DSSDBG("failed to set up scaling, bad source width\n");
				return -EAGAIN;
			}
		}

		if (five_taps) {
			lclk = calc_lclk_five_taps(width, height,
					out_width, out_height, color_mode,
					channel_out);

			/* Try again with 3-tap if possible */
			if (lclk > dispc_lclk_rate() &&
			    height <= out_height * 2) {
				five_taps = false;
				goto again;
			}
		} else
			lclk = calc_lclk(width, height,
					out_width, out_height, channel_out);

		DSSDBG("required lclk rate = %lu Hz\n", lclk);
		DSSDBG("current lclk rate = %lu Hz\n", dispc_lclk_rate());

		if (!lclk || lclk > dispc_lclk_rate()) {
			DSSDBG("failed to set up scaling, "
					"required lclk rate = %lu Hz, "
					"current lclk rate = %lu Hz\n",
					lclk, dispc_lclk_rate());
			return -EAGAIN;
		}
	}
