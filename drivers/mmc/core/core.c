	      ...
	      unsigned int arg)
{
	...
	/*
	 * Workaround Hynix eMMC firmware bug in secure trim by disabling
	 * secure trim for Hynix eMMC's manufactured before 04/2011.
	 */
	if (card->cid.manfid == 0x90 &&
	    (card->cid.year < 2011 ||
	     (card->cid.year == 2011 && card->cid.month < 4)) &&
	    (arg & MMC_TRIM_ARGS) &&
	    (arg & MMC_SECURE_ARGS)) {
		printk(KERN_ERR "Secure trim (%#x %u %u) is broken on "
				"this Hynix eMMC (%#x %02d/%04d)\n",
				arg, from, nr, card->cid.manfid,
...
void mmc_rescan(struct work_struct *work)
{
	...
	if (!err) {
		...
		/* power off in any case, SDIO function drivers should claim power. */
		mmc_power_off(host);
		goto out;
	}
