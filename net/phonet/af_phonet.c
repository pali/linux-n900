...
static uint phonet_gid __read_mostly;
module_param(phonet_gid, uint, 0);
MODULE_PARM_DESC(phonet_gid, "Group id for phonet access control (default 0)");

/* Transport protocol registration */
static struct phonet_protocol *proto_tab[PHONET_NPROTO] __read_mostly;

/* Check credentials required for phonet access */
int phonet_access_check()
{
	return capable(CAP_SYS_ADMIN) || in_group_p(phonet_gid);
}
			...
			struct net_device *orig_dev)
{
	...
	/* learn route to packet source */
	if (skb->pkt_type != PACKET_LOOPBACK)
		phonet_route_input(dev, ph->pn_sdev & 0xfc);
