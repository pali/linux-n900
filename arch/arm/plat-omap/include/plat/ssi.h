...
/**
 * struct omap_ssi_port_data - OMAP SSI port platform data
 * @ready_rx_pad: PAD register offset of ready rx line. DVFS workaround.
 * @ready_tx_pad: PAD register offset of ready tx line. DVFS workaround.
 */
struct omap_ssi_port_pdata {
	u16		ready_rx_pad;
	u16		ready_tx_pad;
};
...
/**
 * struct omap_ssi_port_data - OMAP SSI port platform data
 * @cawake_gpio: GPIO number for the CAWake signal.
 * @ready_rx_gpio: GPIO number sharing the PAD with the ready rx line.
 * @ready_rx_pad: PAD register offset of ready rx line. DVFS workaround.
 * @ready_tx_pad: PAD register offset of ready tx line. DVFS workaround.
 */
struct omap_ssi_port_config {
	unsigned int	cawake_gpio;
	unsigned int	ready_rx_gpio;
	u16		ready_rx_pad;
	u16		ready_tx_pad;
};
...
struct omap_ssi_board_config {
	unsigned int	num_ports;
	struct omap_ssi_port_config *port_config;
};
