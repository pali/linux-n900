...
/* pool_mutex protects static pool reservation */
 ...
 * No return value.
 *
 * As long as the "user->usage = UNUSED;" is the last access to the node
 * mutex is not actually needed. Locking keeps this consistent with
 * the get_user and is it better from code maintenance perspective.
 * It is not that easy to break get_user - free_user pair by accident.
 */
...
int resource_refresh(void)
{
	...
	list_for_each_entry(resp, &res_list, node) {
		mutex_lock_nested(&resp->resource_mutex, resp->subclass);
