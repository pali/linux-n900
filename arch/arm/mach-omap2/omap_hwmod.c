				   ...
				   u32 *v)
{
	...
	if (!oh->class->sysc->sysc_fields) {
		WARN(1, "omap_hwmod: %s: offset struct for sysconfig "
...
{
	struct powerdomain *dep, *source;

	if (!oh->_clk)
		return -EINVAL;

	dep = omap_hwmod_get_pwrdm(oh);
	source = omap_hwmod_get_pwrdm(init_oh);

	if (!dep && !source)
		return -EINVAL;

	return pwrdm_add_sleepdep(dep, source);
}
...
static int _del_initiator_dep(struct omap_hwmod *oh, struct omap_hwmod *init_oh)
{
	...
	return pwrdm_del_sleepdep(dep, source);
}
