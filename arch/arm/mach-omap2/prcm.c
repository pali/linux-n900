...
/* Save emu domain clocks which belongs to mpu subsystem */
void omap3_emu_mpusubsystem_save_context(void)
{
	prcm_context.emu_cm_clkstctrl =
			cm_read_mod_reg(OMAP3430_EMU_MOD, CM_CLKSTCTRL);
}

/* Restore emu domain clocks which belongs to mpu subsystem */
void omap3_emu_mpusubsystem_restore_context(void)
{
